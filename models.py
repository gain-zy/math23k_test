...
class EncoderSeq(nn.Module):
    def __init__(self, input_size, embedding_size, pos_embedding_size, hidden_size, category_size, pretrain_emb,
                 edge_size, n_layers=2, dropout=0.5):
    ...
    def forward(self, input_seqs, input_lengths, cate_word_edge, cate_index_input, cate_length, cate_id_match,
                nums_index, num_pos, batch_graph, edge_graph, hidden=None):
        ...
        if USE_KAS2T_encoder == False:
            ...
            '如何设置相关。 是在group的基础上，添加edge的信息'
            seq_len = input_seqs.size(0)
            node_embed = self.edge_dense1(pade_outputs.transpose(0, 1))  # 384 * 32 : B x S x 32
            h1_embed = node_embed.unsqueeze(2).repeat([1, 1, seq_len, 1])  # B * S * S * 32
            h2_embed = node_embed.unsqueeze(1).repeat([1, seq_len, 1, 1])  # B * S * S * H
            edge_mask = edge_graph.unsqueeze(3).repeat([1, 1, 1, 32])  # B x S x S x 32, 32是因为hidden=32
            '非0的位置，都是embedding，是0的都是0'
            h1_embed = h1_embed.masked_fill_(edge_mask == 0, 0) # B * S * S * 32，S与S的链接，但是不相关的都是0
            h2_embed = h2_embed.masked_fill_(edge_mask == 0, 0) # h1和h2分别2个隐藏向量，用来做gat

            edge_embed = self.edge_embedding(edge_graph)  # B * S * S * E
            edge_embed_cat = torch.cat((edge_embed, h1_embed, h2_embed), 3) # B x S x S x 3E

            edge_graph_expend = edge_embed_cat.permute(0, 3, 1, 2)  # B x 3E x S x S
            # 左边添加0，右边添加max_cat_len, 上面0，下面最大max_cat_len
            ZeroPad = nn.ZeroPad2d(padding=(0, max_cate_len, 0, max_cate_len))  # B * 1 * S + c * S + C
            edge_graph_expend = ZeroPad(edge_graph_expend).permute(0, 2, 3, 1)  # B * 3E * S * S?存疑 S应该不是之前的S了B * S * S * 3E
            ...
            # B x S x H; ? ; B * S * S * 3E
            encoder_outputs_knowledge = torch.stack(
                [att(concat_input_category, cate_word_edge, edge_graph_expend) for att in self.gat_n],
                dim=2)  # B*S*N*head
            ...
            problem_output = F.max_pool1d(output.permute(0, 2, 1).contiguous(), output.shape[1]).squeeze(
                -1)  # (batch_size, d_model,S) (batch_size, d_model)
        ...
        if USE_number_enc == True:
            ...
            for b in range(batch_size):
                ...
                for num_i in range(len(nums_index[b])):
                    ...
                    nums_char_embedded = self.nums_char_embedding(torch.LongTensor(nums_char_list).cuda()).unsqueeze(
                        1)  # num_len*1*E
                    ...
                    nums_gru_final = nums_gru_out[-1, :, :self.pos_embedding_size] + nums_gru_out[0, :,
                                                                                     self.pos_embedding_size:]  # 1*E
        ...
        if USE_number_enc == False:
            ...
            pade_outputs_cat = torch.cat(
                (output.transpose(0, 1), all_problem_hidden, attn_problem_hidden.transpose(0, 1)), 2)  # S*B*H
            problem_output_cat = torch.cat(
                (problem_output, all_problem_hidden[-1, :, :], attn_problem_hidden[:, -1, :]), 1)  # B*H
...
class Prediction(nn.Module):
    ...
    def forward(self, node_stacks, left_childs, encoder_outputs, num_pades, padding_hidden, seq_mask, mask_nums,
                indices, masked_index, question_mask):
        current_embeddings = []
...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, graph_input, adj, edge_embed):
        ...
        #  B * S * S * 3E ->  B * S * S * 1 ->  B * S * S
        middle_result_dis1 = torch.matmul(edge_embed, self.a3).squeeze(3)  # B*S*S*N
        e = self.leakyrelu(middle_result1 + middle_result2 + middle_result_dis1) # 数值相加
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        key = self.linear_k(key).view(nbatches, -1, self.h, self.key_dim).transpose(1,
                                                                                    2)  # B*S*(dim_per_head * num_heads)
        ...
        x, p_attn = attention(query, key, value, mask=mask,
                              dropout=self.dropout)  # (batch_size, h, seq_len, d_k),#B,H,S,S
        if print_dims:
        ...
        if print_dims:
            print("{0}: x (after concatenation and linear): type: {1}, shape: {2}".format(self.__class__.__name__,
                                                                                          x.type(), x.shape))
        return x, p_attn
...
class GATLayer(nn.Module):
    ...
    def forward(self, graph_input, adj):
        ...
        a_input = torch.cat([h.unsqueeze(2).repeat(1, 1, N, 1), h.unsqueeze(1).repeat(1, N, 1, 1)],
                            dim=3)  # B,N,N,2*dim
