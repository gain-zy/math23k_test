...
def train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, 
    ...
    if USE_dependency==True:
        ...
        for j in range(len(input_length)):
            ...
            for tuple_dep in dependency_batch[j]:
                edge_id=depen_vocab.index(tuple_dep[0]) # edge_id
                id_1=tuple_dep[1] # 头
                id_2=tuple_dep[2] # 尾
                edge_id=edge_id+id_used # edge_id + 5 为啥加5 这是因为 0 1 2 3 4 都被占用了
    ...
    encoder_outputs, problem_output,gate_num,num_indicator,just_problem_out,type_num,num_pair_score=encoder(
        input_var,input_length,cate_word_edge,cate_index_input,cate_length,cate_id_match,nums_index,num_pos,
        graph_batch,edge_graph
    )#B*cate+seq*cate+seq
...
for epoch in range(start_epoch,n_epochs):
    ...
    num_size_batches,unit_list_batches,rule3_list_batches,output_middle_batches,\
    input_edge_batches = prepare_train_batch(train_pairs, batch_size, PAD_token)
    print("fold:", fold + 1)
    print("epoch:", epoch + 1)
    start = time.time()
